...
* 타원의 중점 좌표 : `(x,y)`. 
* 타원의 major axis과 minor axis의 길이 : `(major axis length, minor axis length)`.
* 타원의 회전 각도 : `angle`  (**clockwise direction** and **degrees**). 
    * major axis와 x-axis의 각도. 아래 그림 참조할 것.
    * major axis를 `장축` 으로도 지칭하는 경우도 있는데 엄밀히는 조금 틀린 명칭임
    *  major axis가 minor aixs보다 길이가 짧을 수 있음)
...
* `cv2.fitEllipse` 로 contour를 둘러싸는 타원을 계산하여 얻어지는 반환값이 바로 2nd argument로 들어가는 정보임. ref. : [fitEllipse](../ch02/dip_contour_features.md#fitting-an-ellipse)
...
: ^^하나 이상의 다각형^^ 을 color 색상으로 칠한다. `countours=[points_of_contour0, ...]`
