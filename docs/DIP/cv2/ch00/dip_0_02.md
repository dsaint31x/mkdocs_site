...
* 타원의 first axis과 second axis의 길이 : `(first axis length, second axis length)`.
    ...
    * major axis 는 장축을 의미하지만, openCV에서 first axis는 second aixs보다 길이가 짧을 수 있음.
    * first axis 는 ellipse의 회적 각도 `angle`이 0 인 경우 x-axis 가 되는 축임.
    * second axis 는 ellipse의 회적 각도 `angle`이 0 인 경우 y-axis 가 되는 축임.
...
img = cv2.ellipse(img,
                  (256,256), # center
                  (100,50),  # first axis length, second axis length
                  -10,       # rotaion angle (CCW)
                  45,        # start angle (CCW)
                  180,       # end angle (CCW)
...
결과는 다음과 같음: (arc를 그리는 예를 수행한 결과임)
...
<figure markdown>
![](./img/draw_img_ellipse0.png){width=“300” align=“center”}
</figure>
...
닫힌 다각형이 아닌 끝점과 시작점이 연결이 안 된 상태로 그려짐을 확인할 수 있음.
...
<figure markdown>
![](./img/draw_img_polylines0.png){width=“300” align=“center”}
</figure>
