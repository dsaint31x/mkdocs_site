...
colab의 경우, `google.colab.patches` 모듈에서 `cv2_imshow` 함수(cv2.imshow의 alternative)를 대신 제공하며 이 함수는 window가 아닌 출력창에 이미지를 표시함.
...
* [cv_imshow.py](cv/ch00_basic/dip_0_00/cv_imshow.py)
...
`matplotlib`는 차트나 이미지를 나타내는데 가장 널리 사용되고 있는 python library임. (MATLAB 처럼 활용가능한 plotting framework임.)
...
>
> * [cv_matplotlib.py](cv/ch00_basic/dip_0_00/cv_matplotlib.py)
...
* [cv_imwrite.py](https://github.com/dsaint31x/OpenCV_Python_Tutorial/blob/65b2aa86ec168d2a5e9b948b752f1de7161e1b2d/cv/ch00_basic/dip_0_00/cv_imwrite.py)
