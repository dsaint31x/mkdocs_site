...
이같은 방식은 수치적인 정확도 측면에서 장점을 가지나, 연산량이 많아서 가장 근접한 bin에만 반영해주는 경우도 많다.
...
> HOG의 경우, SIFT, SURF, ORG등에 비해서 Local feature를 나타내는 부분은 조금 떨어진다고 알려져있다. (실제로 rotation등에 영향을 받는다.) 하지만, image 전체에 대한 특징으로는 경쟁 기술보다 나은 편이다. 때문에 대상 객체의 자세나 상태가 조금 차이가 나더라도 해당 객체를 image에서 detection하는 등에 많이 사용된다.
...
openCV에서 HOG Descriptor를 생성하기 위해서는 다음의 객체와 메서드를 제공함.
