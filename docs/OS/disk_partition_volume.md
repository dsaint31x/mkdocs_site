...
* Scheme은 전체적인 방식이나 체계를 가리키는 추상적인 표현이고, 
* Schema는 데이터 구조나 문서 구조처럼 일정한 형식을 정의한 구체적인 틀을 가리키는 경우가 많음.

Partitioning Scheme라고 하면, HDD나 storage를 조직화하는 방법으로 이는 컴퓨터에서 데이터를 저장하고 관리하기 위해 사용되는 구조적인 체계를 가리킴.
