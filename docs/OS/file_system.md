...
tags: [OS, File System, Storage, NTFS, FAT32, exFAT, ext4, APFS]
---
# File System

![](./img/file_system.jpg){style="display: block; margin: 0 auto; width:500px"}

File system은 ***OS가 HDD나 SSD 등의 저장장치에 데이터를 저장하고 접근하는 방법을 정의*** 하는 "데이터의 논리적 구조이자 체계"임. 
...
>     * Windows에서는 partition에 file system을 구축(format)하여 OS가 인식가능한 volume으로 만듬.
>     * 즉, volume은 file system으로 관리되는 저장영역에 해당함.
...
1. **file**: ***데이터의 논리적 단위*** 로, 문서, 이미지, 프로그램 등 다양한 형태의 정보를 포함함.
...
file system은 일반적으로 다음과 같은 작업을 지원함:
