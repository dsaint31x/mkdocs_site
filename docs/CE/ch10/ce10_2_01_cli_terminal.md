...
즉, User application이 I/O를 수행하기 위해 

* OS에 **blocking system call** 을 호출하면,
* 커널은 해당 호출을 처리하는 과정에서 데이터가 아직 준비되지 않은 경우 그 process를 sleep 상태로 전환한다.
* 이는 blocking 방식의 buffered input 의 경우임에 유의할 것.

이후 I/O가 진행(I/O 는 CPU가 아닌 device controller + [DMA](https://dsaint31.me/mkdocs_site/CE/ch04/ce04_03_01_io_control/#1-dma-direct-memory-access) 로 수행됨) 되는 동안

* OS는 해당 User application의 process를 sleep 상태로 두고 run queue에서 제거.
* 그 사이 scheduler는 다른 runnable process를 선택하여 CPU에서 실행시킴.

이후 I/O가 완료되면,

* 디바이스는 interrupt를 발생시키고
* 커널은 interrupt handler를 통해 대기 중이던 process를 wake up하여 runnable 상태로 전환.
* 그 후 scheduler의 판단에 따라 context switching이 이루어지면, 해당 process는 core에서 다시 실행된다.

다시 말하면, 

* I/O 종료 event는 interrupt를 통해 커널에 전달되며,
* 그 결과 sleep 상태의 process는 runnable 상태가 된다.
* 다만 interrupt가 즉시 해당 process를 실행시키는 것은 아니며, 실제 실행 시점은 scheduler의 결정에 의해 이루어진다.

실제 동작은 위의 설명보다 복잡하지만 근본적인 부분은 큰 차이 없음.

단, **context switching은 부하가 많이 걸리는 작업** 으로 

*  지나치게 많이 발생할 경우 오히려 효율이 매우 떨어지게 되어 
*  사용자가 컴퓨터가 매우 느리다고 생각할 수 있다.
*  때문에, 이를 효과적으로 수행하기 위해 OS는 다양한 알고리즘을 사용하며 보다 많은 구성요소들(scheduler 등)의 도움을 받는다. 

### 5-1. 사용자의 입력 처리

사용자가 키보드의 키를 누르거나 마우스를 움직이면, 해당 장치는 hardware interrupt를 발생시킴.

* 이 때 CPU는 현재 작업을 잠시 멈추고,
* OS의 ISR(Interrupt Service Routine)을 실행하여 그 입력을 처리한다.

> `Interrupt`에 대한 좀더 자세한 내용은 [Interrupt 요약](https://dsaint31.tistory.com/447)을 참고하라.  

단, 주의할 점이 있다.

* 키를 누를 때마다 hardware interrupt는 발생한다.
* 다만, 입력이 line buffering(예: canonical mode)으로 설정된 경우에는
   * 각 키 입력이 ***커널의 input buffer에 저장*** 될 뿐,
   * 해당 사용자 프로그램이 즉시 깨어나서 실행되는 것은 아니다.
* line buffering의 경우, Enter 키가 눌려 한 줄의 입력이 완성되었을 때 그동안 입력을 기다리며 sleep 상태에 있던 사용자 프로세스가 wake up 된다.

즉,

* (H/W) interrupt는 키를 누를 때마다 발생하고,
* 사용자 프로그램은 입력이 완성되었을 때 다시 실행된다.

이 두 과정은 서로 다른 단계라는 점을 구분하는 것이 중요하다.

> Canonical mode는 터미널 입력을 줄 단위로 버퍼링하여, Enter 키가 눌릴 때까지 사용자 프로세스를 깨우지 않는 입력 처리 방식을 가리킴.
> 터미널입력은 canonical mode이나 게임 등의 경우엔 non-canonical mode(=raw mode)로 동작함.
> 표준적인 이란 뜻에서 canonical mode라고 불리며, CLI 에서 명령어 입력 중 수정이 가능한 이유에 해당함(엔터를 누르기 전 수정이 가능)
...
해당 I/O가 확실히 이루어지고 나서 다음 명령어를 실행해야하는 경우라면, 

* 해당 system call에 대한 응답을 대기(blocking)하면서
* 해당 process를 멈추고 있는 ***block mode function*** 으로 I/O를 처리하는게 맞지만, 

I/O에 상관없이 다음 구문을 수행할 수 있는 경우라면  

* I/O작업과 process 수행이 병렬로 이루어지도록
* ***non-blocking mode function*** 으로 I/O처리하는 것을 고려해볼 수 있다.
