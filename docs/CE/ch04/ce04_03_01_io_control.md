...
컴퓨터와 입출력 장치가 데이터를 주고받는 방법을 **입출력 제어방식 (Input/Output Control Methods)** 이라고 함.
...
- **특징 (Characteristics)**:
  ...
  - CPU가 I/O Device의 상태 Register를 읽어 장비가 준비되었는지를 확인.
...
- **상태 레지스터 (Status Register)**:
  ...
  - **위치 (Location)**: I/O 장치의 컨트롤러 내부에 위치하며, CPU 내부에 존재하지 않음.
