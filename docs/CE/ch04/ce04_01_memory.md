...
x86-64에서는 일반적으로 48bit (Level-4 Paging) 또는 최근에는 57bit virtual address (Level-5 Paging)를 지원함.
...
>
> $2^{64}$ byte = $2^{64-40}$ TiB (tebibyte)= $2^{64-50}$ PiB (pebibyte) = $2^{64-60}$ EiB (exbibyte)
