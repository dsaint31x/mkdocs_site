...
summary: Python의 module, package, namespace package의 개념과 import 방식의 기본 동작을 정리함.
tags: [python, module, package, import, namespace-package]
categories: [Python]
toc: true
public: true
updated: 2026-05-17
...
> 위의 설명은 초보자 대상의 간단한 설명임.
> 엄밀히는 built-in module, extension module, package 들도
> Python의 import system에서 `module` 이라고 봄.
...
* **`builtins` module** 을 제공하고 있음.
* (Python의 built-in function들은 `__builtins__` 라는 이름으로 노출되는 builtins 모듈에 속함)  
...
> 참고로 `__name__` 은 module object에 설정되는 **special global variable** 임.
...
위의 import는 explicit import라고 불리며,  
전체 모듈을 import 하는 방법임.  
import를 하는 보다 자세한 방법은 다음 글을 참고:

* [[Python] import 의 종류](https://ds31x.tistory.com/425)
...
* 때문에 특정 Python Interpreter Session이 수행되고 있는 동안에  
  특정 module의 소스코드 자체가 바뀌더라도 해당 변경사항이 반영되지 않는다.  
  
이를 위해서는 `importlib` 모듈의 `reload(모듈명)` function을 이용해야 한다.  
자세한 사용법은 다음 글을 참고하라.  

* [importlib.reload : module 재적재](https://dsaint31.tistory.com/527)
...
> 실제 module에 해당하는 file에서 수정이 일어나진 않고,  
> 단순히 memory상에서 `math`의 namespace의 `pi`라는 이름이 가리키는 object가 바뀐 것이므로  
> 해당 Python interpreter의 session에서만 `72`라는 값을 의미하게 되고, 다른 `math` 모듈을 사용하는 session에서는 문제가 없다.
...
> aliased import 라고 불림.

앞서 예의 `import` statement에서 import된 모듈에 접근은 
***모듈 파일의 name(이름)*** 을 통해서 이루어짐.
...
> selective import 라고 불림.

현재 `ds_cal.py` 모듈에서 필요한 attribute는 바로 `ds_addition` 함수 뿐이라고 가정하자.
...
* `from 모듈명 import Attribute0, Attribute1`과 같이  
...
> global import 또는 wildcard import라고 불림.
...
>
> * module에 `__all__`이 정의되어 있으면 `__all__`에 들어 있는 이름들이 import됨.
> * `__all__`이 없으면 `_`로 시작하는 이름을 가지는 attribute들은 import가 이루어지지 않음.
...
> Python에서 `package`는  
> 여러 module을 하나의 계층적 namespace 아래에 묶어 관리할 수 있게 해주는  
> directory 기반의 `module`

Python에서 `module`이 `.py`파일로 구현되는 것처럼,  
`package`는 `module`들이 포함된 directory로 구현된다.

> Python 2.x 의 경우, `__init__.py`가 있어야 했으나  
> Python 3.x에서는 없어도 namespace package로 처리되어 동작함.  
>  
> 하지만 호환성을 위해 빈 파일로라도 `__init__.py`를 넣어주는 게 좋음.

`package`를 통해 `module`들은 

* logically 비슷한 것들끼리 구분짓고 
* ***계층화*** 할 수 있다.

다음 예제는  
사칙연산을 구현한 `ds_cal.py`를 `farith` package에 두고,  
기본 삼각함수 관련 함수를 제공하는 `ds_cal.py`를 `ftri` package로 나누어 제공하면서  
이들을 `ds_run03.py`에서 모듈로 사용하는 것을 보여줌.
...
* 각 namespace package portion의 parent directory가 `sys.path`에 포함되어 있고  
