...
만약 process와 thread의 개념이 명확하지 않다면 다음 URL을 참고할 것: [Process vs Thread](https://ds31x.tistory.com/152)
...
PyQt와 PySide는 multithread 및 multiprocess 구현을 지원하는 다양한 class와 도구를 제공한다. 이들을 활용함으로써 개발자는 현대 computing 환경의 이점을 활용하여, 효율적이고 강력한 GUI application을 제작할 수 있다.
