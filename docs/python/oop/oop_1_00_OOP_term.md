...
`Modularity` 
: 프로그램을  
좀 더 이해하기 쉽고, 작고 관리가 쉬우면서, 서로 interaction하는 조각 으로 나누어 개발하는 방법 또는  
...
`Encapsulation`  \*\*\*
: Object에서 ^^중요한 데이터와 세부적인 구현 방법에 대한 사항들을  capsule 로 싸는 형태^^ 를 취함으로  
해당 데이터와 구현방법에 대한 외부로부터의 직접 접근을 막고,  
대신 외부에서 이들을 조작할 수 있는 방법인 **interface 만을 공개** 하는 기법을 가리킴.
...
`Message Passing` 
: Event-Driven, Message-Driven, Sending Message 라고도 불리는 개념으로  
Object간의 interaction 방식을 지칭한다.

`Polymorphism` \*\*\*
: Inheritance를 통해 이루어지는 개념 중 하나로  
***동일 message로 여러 종류의 object에게 각각 고유한 task를 수행*** 시킬 수 있게 해줌.
