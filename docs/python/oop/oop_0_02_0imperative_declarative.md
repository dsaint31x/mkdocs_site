...
^^소스 코드에서는 해당 결과를 얻기 위해 어떻게 처리가 이루어질지는 기술하지 않는다.^^
...
즉, 실행(execute)할 instruction들을 순서대로 기술한다. 

프로그래머는 각각의 instruction들이 어떤 조건과 순서로 동작할지를 기술하며 다음을 명시적으로 소스코드에 기재한다.

Imperative Programming Language의 조건은 다음을 지원하는 것임.
...
2. loops (iteraton, recursion)
3. conditions (=conditional branching)

위의 조건은 imperative programming language의 조건이라기 보다는 범용 Programming Language가 Turing Completeness를 만족하기 위한 조건임.  
하지만 범용 imperative programming language에서 대부분 제공하는 기능이라 여기서 다룸.
