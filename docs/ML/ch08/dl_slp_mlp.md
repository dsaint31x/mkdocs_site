...
* $P(y \mid x)$: 입력 $x$ 가 주어졌을 때 true label $y$가 관측될 likelihood.

이 likelihood에 negative log를 취하면 다음과 같음.

$$
\mathcal{L}_{\mathrm{NLL}}(y, \hat{p}) = -\log (P(y \mid x)) =
-\left[
...
* $\hat{p}$ : model이 예측한 positive class probability. 즉, $\hat{p}=P(y=1 \mid x)$.
* $1-\hat{p}$ : negative class probability. 즉, $P(y=0 \mid x)$.
...
\mathcal{L}_{\mathrm{log\ loss}}(y, \hat{p})
=
