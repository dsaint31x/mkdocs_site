...
이를 반영한 Learning Rate Scheduling의 핵심 아이디어는 다음과 같음.
...
| 분류                            | 핵심 기준                                             | 대표 기법                                                    |
| ----------------------------- | ------------------------------------------------- | -------------------------------------------------------- |
| Monotonic Decay Scheduling    | training이 진행될수록<br/>learning rate를 전반적으로 감소시킴       | Exponential, Power, Piecewise Constant, Cosine Annealing |
...
| Warm-up + Decay Scheduling    | 초반에는 learning rate를 증가시키고,<br/>이후에는 다시 감소시킴           | Warm-up + decay, 1cycle                                  |
| Cyclical / Restart Scheduling | learning rate 증가와 감소를<br/>여러 번 반복하거나 restart 시켜<br/> plateau나 local optimum 탈출 유도 | Cyclical LR, Cosine Annealing with Warm Restarts         |
...
Warm-up 이후에는 다른 scheduler를 사용하는 방식임.
...
| 상황                               | 권장 방식                                    |
| -------------------------------- | ---------------------------------------- |
| 단순하고 안정적인 training               | Exponential Scheduling, Cosine Annealing |
| 직접 epoch별 감소 시점을 정하고 싶음          | Piecewise Constant Scheduling            |
| validation 성능이 정체될 때 자동으로 줄이고 싶음 | Performance Scheduling                   |
| 초반 training이 불안정함                | Warm-up + Decay                          |
| 빠른 학습과 좋은 일반화 성능을 동시에 노림         | 1cycle Scheduling                        |
| plateau나 local optimum에서 벗어나고 싶음 | Cyclical LR, Cosine Warm Restarts        |
