...
이를 해결하기 위한 방안 중 하나가 k-fold cross validation으로, training dataset을 k개의 subset으로 나누고, 각 subset을 validation set으로 삼아 나머지로 학습을 한 model에 대해 evaluation하는 것을 k번 반복함으로서 evaluation의 결과의 신뢰도를 확보한다.
