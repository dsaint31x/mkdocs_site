...
MSE objective는 parameters에 대해 convex이므로,  
Gradient Descent는 적절한 learning rate 또는 step size와 충분한 iteration 아래 global minimizer 중 하나에 수렴할 수 있음.  
한편, SVD 기반 Linear Regression (pseudo inverse를 구하는데 SVD사용)의 계산 복잡도는  
일반적으로 design matrix $X \in \mathbb{R}^{m \times n}$ 에 대해 $O(mn^2)$ 수준이므로,   
sample 수 $m$와 feature 수 $n$가 매우 큰 경우에는  
iterative optimization method인 GD 계열 (한 epoch 기준으로 $O(mn)$의 계산량을 가짐)이 더 실용적일 수 있음.

* 참고: [Graident Descent Method](https://dsaint31.tistory.com/633)
