...
GD를 이용해도 MSE loss가 parameters에 대해 convex하므로 적절한 learning rate와 충분한 iteration인 경우 항상 global minimizers 중 하나로 수렴 가능함.
SVD에 기반한 Linear Regression의 계산 복잡도는 $O(n^2)$ 정도이기 때문에 features의 갯수 $n$인 매우 큰 경우엔 GD를 사용하는 것이 보다 나음.​
