...
date: 2026-04-02
description: "Batch Normalization의 개념, 알고리즘, 장단점, ICS 가정의 한계와 현재 해석을 정리."
slug: "batch-normalization-overview"
tags:
  - batch normalization
  - bn
  - deep learning
...
* optimization landscape smoothing (regularization효과)이 이루어지기 때문에

좋은 성능을 보이는 것으로 생각하는게 현재 시점에서는 보다 나은 해석으로 받아들여짐.

![](./img/bn_regularization.png){style="display:block; margin: 0 auto; width: 800px"}
...
* RNN 등에서는 Time마다 통계적으로 다른 분포의 input을 가지기 때문에 `BN`의 적용이 쉽지 않음 
    * 때문에 RNN등에서는 feature에 대해 적용되는 `BN`이 아닌 단순한 `gradient clipping`이나 `sample`에 대해 적용되는 [***`layer normalization`이 대신 사용됨.***](https://ds31x.github.io/wiki/review/LayerNormalization/)
...
`BN`은 매우 효과적인 optimization을 가능하게 하기 때문에,  
...
![](./img/bn_inference.png){style="display:block; margin: 0 auto; width: 800px"}
...
$$\begin{aligned}\hat{\textbf{v}} &\leftarrow \hat{\textbf{v}} \times \text{momentum} + \textbf{v} \times (1-\text{momentum}), \quad \text{EMA} \\\\
\hat{\textbf{v}} &\leftarrow \textbf{v} \times \text{momentum} + \hat{\textbf{v}} \times (1-\text{momentum}), \quad \text{PyTorch}
$$
