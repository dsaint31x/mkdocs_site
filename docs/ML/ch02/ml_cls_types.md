...
* Multiclass classification (다중클래스분류)와 Multilabel classification (다중라벨분류) 등의 기본이 되는 task.
...
* `Binary Classification` 의 generalization으로
* 한 sample에 하나의 label값이 주어지지만, 해당 label값의 종류가 여러 class인 경우를 애기함.

`학점` 을 classification하는 것이 대표적인 예라고 할 수 있음.

* Gaussian Naive Bayesian 이나 Logistic Regression, Random Forest 등은 Multiclass classification task로 구현상 자연스럽게 확장이 가능하지만,
* 기본적으로 binary classification 만이 가능한 Support Vector Machine Classifier나 Stochastic Gradient Descent Classifier는 One-vs-One (`OvO`) 또는 One-vs-Rest (`OvR`) 등의 방식으로 여러 binary classification model을 통해 Multiclass classification을 수행함.
...
* 이 경우 2개의 elements로 구성된 label vector가 사용되고,
* 각 elements는 0,1 (or False, True)의 2가지 값 중 하나를 가지게 됨.

또는 특정 사진에 2개의 인물 A와 B가 있는 경우, 해당 사진은 A 가 포함되었는지를 나타내는 binary classification과 B가 포함되었는지를 나타내는 binary classification이 동시에 이루어지게 구현될 수 있으며 이 경우 **Multilabel classification** 이라고 함.
