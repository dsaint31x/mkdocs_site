...
$$\textbf{Accuracy} = \frac{\textbf{Correct\_Predictions}}{\textbf{All\_Predictions}}$$

Binary classification으로 말한다면

$$\textbf{Accuracy} = \frac{TP+TN}{TP+FP+TN+FN}$$

* $TP$ : True Positive (실제로 Label이 Positive(1)인데 Positive라고 predict한 갯수)
* $TN$ : True Negative (실제로 Label이 Negative(0)인데 Negative라고 predict한 갯수)
* $FP$ : False Positive (실제로 Label이 Negative(0)인데 Positive라고 predict한 갯수)
* $FN$ : False Negative (실제로 Label이 Positive(1)인데 Negative라고 predict한 갯수)
...
> * `전체 샘플의 수 = TP의 합 + FP의 합 = TP의 합 + FN의 합` 이 성립.
...
예를 들어, 하나의 샘플이 5개의 라벨을 가질 수 있다고 가정할 때, 이 샘플에 대해 모델이 예측한 라벨 중 실제로 올바른 경우의 비율을 구할 수 있음.
...
Multiclass classification 에서도 계산 가능함.
...
$$
\textbf{FPR}=\frac{FP}{FP+TN}= 1-\frac{TN}{FP+TN} = 1-\textbf{specificity}
$$
...
>
> 사실, PR-Curve는 ROC-Curve 보다 항상 비관적으로 모델의 성능을 평가함.
...
> 
> * ROC Curve는 False Positive가 증가(=모델의 성능이 나뻐짐)하더라도, Negative Label의 수가 커서 FPR의 변화량이 크지 않아 AUC에서 큰 차이를 보이지 않음.  
...
$$
\begin{aligned}F_{\beta}=F&=\frac{1}{\alpha\frac{1}{\textbf{precision}}+(1-\alpha)\frac{1}{\textbf{recall}}}\\ &=\frac{(\beta^2+1)\textbf{precision}\times\textbf{recall}}{\beta^2\textbf{precision}+\textbf{recall}}\end{aligned}
$$
