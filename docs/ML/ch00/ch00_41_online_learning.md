...
description: >
  Input data stream으로부터 점진적(incremental)으로 학습하는 
  Online Learning의 개념, 특징, 그리고 Out-of-core Learning과의 관계를 설명한다.
tags:
  - online learning
...
![](../img/ch00/online_learning.png){style="display: block; margin: 0 auto; width:400px"}
...
Online Learning의 방식은 

1. Input data stream으로부터 **점진적 학습(incremental learning)** 을 수행하므로,
2. 일부 데이터를 통해 현재의 파라메터를 갱신하는 학습이 가능함.
3. 이 같은 방식은 전체 데이터 크기가 너무 커서 한번에 학습시킬 수 없을 때 (memory나 storage등에 한번에 적재가 불가능한 경우)에도 적용가능.
4. 즉, 전체 데이터를 다 사용하지 않고 이를 분리하고 이들 분리된 데이터들을 통해 점진적 학습을 수행할 수 있음.
5. 이 같이 사용되는 경우는 **out-of-core learning** 이라고도 불린다. 
...
이에 대한 대표적인 예가 Decision Tree 와 이에 기반한 Random Forest, Gradient Boosting 등과 Kernel SVM 임. 
