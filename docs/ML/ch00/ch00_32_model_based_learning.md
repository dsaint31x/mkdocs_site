...
* 만일 model이 만들어지지 않는 [instance based learning](./ch00_31_instance_based_learning.md)이라면,
* 사각형으로 prediction될 것이다. (`k=3`인 k-NN이라면)
...
> 
> * **input features와 label간의 관계에 대한 approximation** 을 생성하는 **알고리즘(algorithm)** 또는 **방법(method)**
...
1. Data 분석
2. 해당 Training data에 적절한 model을 선택.
3. 해당 model을 training dataset으로 학습 (cost func. 을 최소화시키는 optimization)
4. 이후 inference 수행 (새로운 data point들에 대한 prediction)

## Example

Model based Learning과 Instance based learning의 간단한 실습용 예제:
